...
class IptvProvider extends ChangeNotifier {
  ...
  Future<void> addFutbolTotalBridgeSource(String name, String url) async {
    final clean = url.trim();
    if (clean.isEmpty) return;
    final playlist = Playlist(
      id: 'futbol-total-${DateTime.now().microsecondsSinceEpoch}',
      name: name.trim().isEmpty ? 'Fútbol Total · Puente' : name.trim(),
      source: clean,
      isRemote: true,
      channels: const [],
      lastUpdated: DateTime.now(),
      sourceType: PlaylistSourceType.futbolTotalBridge,
    );
    _playlists = [..._playlists, playlist];
    _selectedPlaylistId ??= playlist.id;
    await _localStore.saveServices(_playlists);
  ...
  Future<void> updateFutbolTotalBridgeSource({
    required String playlistId,
    required String name,
    required String url,
  }) async {

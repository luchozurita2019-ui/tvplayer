...
class M3uImporter {
    ...
    fun downloadAndParse(sourceId: String, url: String): Result {
        ...
        require(!looksLikeXtreamGetPhp(url)) {
            "La URL es un get.php de Xtream y debe cargarse mediante player_api.php"
        }
        ...
        text.lineSequence().forEach { raw ->
            ...
            when {
                ...
                pendingInfo != null -> {
                    ...
                    val displayName = info
                        .substringAfter(',', attrs["tvg-name"].orEmpty())
                        .trim()
                        .ifBlank { "Sin nombre" }
                    ...
                    val seriesInfo = if (section == ContentSection.SERIES) {
                        inferSeries(displayName, group)
                    } else {
                        null
                    }
                    val item = CatalogItem(
                        ...
                        extension = line.substringBefore('?')
                            .substringAfterLast('.', "")
    ...
    private fun download(url: String, redirectsLeft: Int = 5): String {
        ...
        if (status in REDIRECT_CODES) {
            ...
            conn.disconnect()
            if (next.isNullOrBlank() || redirectsLeft <= 0) {
                error("Redirección M3U inválida")
            }
    ...
    private fun parseAttributes(extinf: String): Map<String, String> {
        ...
        ATTR.findAll(extinf.substringBefore(',')).forEach { match ->
    ...
    private fun inferSeries(name: String, group: String): Triple<String, Int?, Int?> {
        ...
        val base = if (match == null) {
            name
        } else {
            name.removeRange(match.range).trim(' ', '-', '.', ':')
        }
    ...
    companion object {
        ...
        /**
         * Xtream's get.php is a playlist endpoint, but when username/password are
         * present it represents an Xtream account. Treating it as a generic M3U
         * destroys the native category/Series structure, so callers must route it
         * through player_api.php instead.
         */
        fun looksLikeXtreamGetPhp(raw: String): Boolean {
            if (raw.isBlank()) return false
            return runCatching {
                val url = URL(raw.trim())
                val path = url.path.orEmpty().lowercase(Locale.ROOT).trimEnd('/')
                if (!path.endsWith("get.php")) return@runCatching false
                val params = url.query.orEmpty().split('&').mapNotNull { part ->
                    val pos = part.indexOf('=')
                    if (pos <= 0) return@mapNotNull null
                    val key = URLDecoder.decode(part.substring(0, pos), "UTF-8")
                        .lowercase(Locale.ROOT)
                    val value = URLDecoder.decode(part.substring(pos + 1), "UTF-8")
                    key to value
                }.toMap()
                params["username"].orEmpty().isNotBlank() &&
                    params["password"].orEmpty().isNotBlank()
            }.getOrDefault(false)
        }

        private fun stableKey(value: String): String = value.trim()
            .lowercase(Locale.ROOT)
